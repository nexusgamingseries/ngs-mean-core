...
export class CasterReportComponent implements OnInit {
  ...
  casterSelected(caster, type) {
    if (type == "cocaster") {
      if (
        this.casterReport.coCasters.indexOf(caster.displayName) == -1 &&
        caster.displayName.length > 0
      ) {
  ...
  addLink() {
    if (
      this.casterReport.vodLinks.indexOf(this.vodLink) == -1 &&
      this.vodLink.length > 0
    ) {
  ...
  ngOnInit(): void {
    this.AR.paramMap.subscribe((params) => {
      ...
      this.casterReport.division =
        params["params"].division != "undefined"
    ...
    this.User.getCasters().subscribe(
      (casters) => {
        ...
        this.filteredOptions = this.coCastCtrl.valueChanges.pipe(
          startWith(""),
