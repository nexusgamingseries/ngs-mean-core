...
export class ChallongeTournComponent implements OnInit {
    ...
    if (val && !this._team) {
      ...
      if (val.cupDiv) {
        ...
        this.scheduleService
          .getTournamentGames(null, this.season, this._division.divisionConcat)
          .subscribe(
            (res) => {
  ...
  getTournLink() {
    if (this.selectedCup) {
      this._tournamentLink =
        this.localStoreTournaments[this.selectedCup].challonge_url;
      this.challonge();
    }
  }
  ...
  ngOnInit() {
    ...
    this.matchWidthMultiplier = this.matchWidthMultiplier
      ? this.matchWidthMultiplier
      : "1.5";
  ...
  challonge() {
    if (this._tournamentLink) {
      $(".tournament").challonge(this._tournamentLink, {
        subdomain: "",
        theme: this.theme,
        multiplier: this.multiplier,
        match_width_multiplier: this.matchWidthMultiplier,
        show_final_results: this.showFinalResults,
        show_standings: this.showStandings,
      });
    }
  }
}
