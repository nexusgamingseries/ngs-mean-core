...
@Component({
  ...
  standalone: true,
  imports:[MatAutocompleteModule, ReactiveFormsModule, FormsModule, MatLabel, MatFormField]
})
export class UserSearchComponent implements OnInit {
  ...
  constructor(private users: UserService, private Auth: AuthService) {
    this.userCtrl.valueChanges.subscribe((data) => {
  ...
  ngOnInit() {
    ...
    setInterval(() => {
      if (this.unresolved) {
        this.users
          .userSearch(this.userCtrl.value, this.userSearchType)
          .subscribe((res) => {
            if (res.length == 0) {
              this.userCtrl.setErrors({ noMatch: true });
            }
