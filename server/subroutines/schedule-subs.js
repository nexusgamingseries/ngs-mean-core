 ...
 * @param {number} season 
 */
async function generateSeasonTwo(season) {
...
/**
 * @name generateRoundRobinScheduleTwo
 * @function
 * @description generates a round robin schedule
 * @param {number} season 
 */
async function generateRoundRobinScheduleTwo(season) {
    let logObj = {};
...
/**
 * @name generateRoundRobinChallonge
 * @function
 * @description generates round robin events calling upon the powers of challonge
 ...
 */
async function generateRoundRobinChallonge(teams, season, division, eventType) {

    let returnObj = {};
    var type = ROUND_ROBIN;
    ...
    if (newTournament.errors && newTournament.errors.length > 0) {
        ...
        if (addParticipants.errors && addParticipants.errors.length > 0) {
            ...
            if (startStatus.errors && startStatus.errors.length > 0) {
                ...
                throw (new Error('Start Tournament Error'));
            } else {
                ...
                if (showTournament.errors && showTournament.errors.length > 0) {
                    ...
                    throw (new Error('Tournament Show Error'));
                } else {
...
module.exports = {
    generateSeasonTwo: generateSeasonTwo,
    generateRoundRobinScheduleTwo: generateRoundRobinScheduleTwo,
